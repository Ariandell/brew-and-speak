...
export const TeacherStatistics = () => {
    ...
    return (
        <ProductPage>
            ...
            <div className="mt-3 space-y-3">{courseRows.map(row => <Card key={row.course.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-[16px] font-black">{row.course.title}</h3><p className="mt-1 text-[11px] font-semibold text-text-soft">{row.students} учнів · {row.lessonCount} уроків</p></div>{row.course.level && <StatusPill>{row.course.level}</StatusPill>}</div><div className="mt-4 flex justify-between border-t border-line pt-3 text-[11px] font-bold text-text-soft"><span>Сумарно завершено</span><strong className="text-text">{row.completed}</strong></div></Card>)}</div>
        </ProductPage>
    );
};
