    ...
    return (
        <ProductPage>
            ...
            <Card className="mt-4 space-y-4 p-5">
                <Field label={locked ? 'Твоя відповідь' : 'Відповідь'}>
                    <textarea value={answer} disabled={locked} onChange={event => setAnswer(event.currentTarget.value)} rows={7} placeholder="Напиши відповідь тут…" className={`${inputClass} resize-none`} />
                </Field>
                <Field label="Файл" hint="до 20 МБ">
                    <input type="file" disabled={locked || busy} accept="image/png,image/jpeg,image/webp,image/gif,audio/*,video/*,.pdf,.doc,.docx" onChange={event => {
